...
export function applyItemToLine(item: Item, quantity = 1): Omit<LineItem, 'id'> {
  return {
    itemId: item.id,
    itemCode: item.code,
    ...
    amount: quantity * item.defaultUnitPrice,
  }
}

export function recalcLineAmount(line: Pick<LineItem, 'quantity' | 'unitPrice'>): number {
